#!/usr/bin/python3
"""This contains an entry point of a command interpreter"""
import cmd
import re
...
# The global constant since the both functions in and outside use it.
CLASSES = [
    "BaseModel",
    "User",
    "City",
...
class HBNBCommand(cmd.Cmd):
    """A class that implements a console
    for AirBnB clone web application
    """
    prompt = "(hbnb) "
    storage = models.storage

    def emptyline(self):
        """The command to be executed when the empty line + <ENTER> key"""
        pass

    def default(self, arg):
        """The default behaviour for the cmd module when the input is invalid"""
        action_map = {
            "all": self.do_all,
            "show": self.do_show,
    ...
    def do_EOF(self, argv):
        """For EOF signal to exit a program"""
        print("")
        return True

    def do_quit(self, argv):
        """If executed, exits a console."""
        return True
    ...
    def do_show(self, argv):
        """Print a string representation of the instance based
        on a class name and id"""
        args = check_args(argv)
        if args:
            if len(args) != 2:
    ...
    def do_all(self, argv):
        """Print all the string representation of every instances based or not
        based in a class name"""
        arg_list = split(argv)
    ...
    def do_destroy(self, argv):
        """Deletes the class instance based on a name and the given id."""
    ...
    def do_update(self, argv):
        """Update the instance based on a class name and the id by adding or
        updating the attribute then save it to JSON file."""
        arg_list = check_args(argv)
        if arg_list:
            if len(arg_list) == 1:
                print("** instance id missing **")
    ...
    def do_count(self, arg):
        """Retrieves a number of instances of the class"""
        arg1 = parse(arg)
        count = 0
